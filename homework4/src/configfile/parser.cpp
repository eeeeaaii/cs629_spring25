...
void parseSphere(std::vector<std::string> attributes, RenderingInfo* rinfo) {
	...
	for (auto attribute : attributes) {
		if (startsNumericAttribute(attribute)) {
			...
			std::string name = getAttributeName(attribute);
			if (name == "$mat") {
				sphere->material_name = getStringAttributeValue(attribute);
			} else {
...
void parseTriangle(std::vector<std::string> attributes, RenderingInfo* rinfo) {
	...
	for (auto attribute : attributes) {
		if (startsVectorAttribute(attribute)) {
			...
			std::string name = getAttributeName(attribute);
			if (name == "$mat") {
				triangle->material_name = getStringAttributeValue(attribute);
			} else {
				throw "Unrecognized string attribute: " + name;
			}
		} else {
...
RenderingInfo* Parser::parseFile(std::string filename) {
	...
	for (auto sphere : rinfo->spheres) {
		if (material_map.find(sphere->material_name) == material_map.end()) {
			throw "Material " + sphere->material_name + " not found";
		}
		sphere->material = material_map[sphere->material_name];
	}
	for (auto triangle : rinfo->triangles) {
		if (material_map.find(triangle->material_name) == material_map.end()) {
			throw "Material " + triangle->material_name + " not found";
		}
		triangle->material = material_map[triangle->material_name];
	}
	return rinfo;
}